...
function renderOffers() {
  ...
  for (const [key, label] of GROUPS) {
    ...
    for (const offer of group) {
      ...
      const button = fragment.querySelector(".claim-record-button"); const claimed = Boolean(state.claims.records[offer.offer_id]); button.textContent = claimed ? "✓ 已领取" : "标记已领取"; button.classList.toggle("is-claimed", claimed); button.disabled = state.busy; button.addEventListener("click", () => toggleClaim(offer)); cards.append(fragment);
    }
