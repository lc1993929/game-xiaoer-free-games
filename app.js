...
function formatDate(value, timezone = "Asia/Shanghai") { if (!value) return "截止时间未公布"; return new Intl.DateTimeFormat("zh-CN", { timeZone: timezone, month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(value)); }
...
function countdown(value) { if (!value) return "记得尽早领取"; const ms = new Date(value).valueOf() - Date.now(); if (ms <= 0) return "已经结束"; const hours = Math.floor(ms / 3600000); return hours >= 24 ? `还剩 ${Math.floor(hours / 24)} 天` : `仅剩 ${Math.max(1, hours)} 小时`; }
...
function renderOffers() {
  ...
  for (const [key, label] of GROUPS) {
    ...
    for (const offer of group) {
      ...
      fragment.querySelector(".platform").textContent = offer.platform_label || offer.platform; fragment.querySelector(".region").textContent = (offer.regions || []).join(" / "); fragment.querySelector(".type").textContent = TYPES[offer.offer_type] || offer.offer_type; fragment.querySelector(".title").textContent = offer.title; fragment.querySelector(".ownership").textContent = OWNERSHIP[offer.ownership] || offer.ownership; fragment.querySelector(".membership").textContent = offer.membership_required ? "需要会员" : "无需会员"; fragment.querySelector(".ends-at").textContent = offer.ends_at ? `${formatDate(offer.ends_at, offer.timezone)} 截止` : "截止时间未公布"; fragment.querySelector(".countdown").textContent = countdown(offer.ends_at); fragment.querySelector(".claim-link").href = offer.official_url;
      const button = fragment.querySelector(".claim-record-button"); const label = fragment.querySelector(".claim-state-label"); const claimed = Boolean(state.claims.records[offer.offer_id]);
      label.textContent = claimed ? "已领取" : "标记为已领取"; button.classList.toggle("is-claimed", claimed); button.setAttribute("aria-pressed", String(claimed)); button.setAttribute("aria-label", claimed ? `撤销 ${offer.title} 的已领取记录` : `将 ${offer.title} 标记为已领取`); button.disabled = state.busy; button.addEventListener("click", () => toggleClaim(offer)); cards.append(fragment);
    }
